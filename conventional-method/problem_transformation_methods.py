...
def main():
    # Read Data
    dataname = "scene"
    path = "/Users/admin/Dropbox/Thesis/Masters_Thesis/experimental_data/{}".format(dataname)
    X = np.array(pd.read_csv(path + "/{}_features.csv".format(dataname)))
    y = np.array(pd.read_csv(path + "/{}_labels.csv".format(dataname)))
    n_samples = X.shape[0]
    ...
    c = 1
    g = 0.1
    inner_clf = LogisticRegression(C=c)
    #clf = LabelPowersets(inner_clf)
    clf = BinaryRelevance(inner_clf)
    #clf = ClassifierChains(inner_clf)
    #clf = MetaStacking(svm.SVC(kernel='rbf', C=c, gamma=g))
    #clf = SubsetMapping(svm.SVC(kernel='rbf', C=c, gamma=g))

    # Fit
    clf.fit(X_train, y_train)

    # Predict
    #prior_k = generate_prior_knowledge(y_test, 2, same=False, seed=58)
    #pred = clf.predict(X_test, prior_knowledge=prior_k)
